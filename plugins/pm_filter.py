...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("🙃 നാണം ഉണ്ടോടോ വെല്ലോരുടെ പ്രോപ്പർട്ടിയിൽ പിടിച്ചു കുത്താൻ....", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("എവിടെ ആയിരുന്നു ഇത്രേം നേരം ലിങ്ക് പോയി ഒന്നുടെ റിക്വസ്റ്റ് ചെയ്തോ.🙃", show_alert=True)
        return
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('MOVIE/SERIES DOESNT FOUND  ❌️')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("🙃 നാണം ഉണ്ടോടോ വെല്ലോരുടെ പ്രോപ്പർട്ടിയിൽ പിടിച്ചു കുത്താൻ....", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        if int(clicked) == typed:
            ...
            return await query.answer(f"{query.from_user.first_name}!🙃 നാണം ഉണ്ടോടോ വെല്ലോരുടെ പ്രോപ്പർട്ടിയിൽ പിടിച്ചു കുത്താൻ....", show_alert=True)

    elif query.data.startswith("checksub"):
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("വെളച്ചിൽ എടുക്കരുത് കേട്ടോ!", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
